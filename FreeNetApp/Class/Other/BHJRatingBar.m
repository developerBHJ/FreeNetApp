...
@property (nonatomic,weak) id<BHJRatingBarDelegate> delegate;

@end

@implementation BHJRatingBar
 ...
 *  @param delegate          代理
 */
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<BHJRatingBarDelegate>)delegate{
    ...
    //星星图片之间的间距
    CGFloat space = (CGFloat)(viewWidth - width*5)/16;
...
- (void)touchesRating:(NSSet *)touches{
    ...
    //星星图片之间的间距
    CGFloat space = (CGFloat)(self.frame.size.width - width*5)/16;
    
    float newRating = 0;
    
    if (point.x >= 0 && point.x <= self.frame.size.width) {
        if (self.isInteger) {
            if (point.x < space*2+width){
                newRating = 1.0;
