...
- (void)setupUI
{
    ...
    [self.arrayRoot enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.arrayProvince addObject:obj[@"province_name"]];
    }];
    ...
    [citys enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.arrayCity addObject:obj[@"city_name"]];
    }];

    NSMutableArray *mArray = [NSMutableArray array];
    NSArray *area = [citys firstObject][@"area"];
    for (NSDictionary *mDic in area) {
        [mArray addObject:mDic[@"name"]];
    }
    self.arrayArea = mArray;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        self.arrayArea = [NSMutableArray arrayWithArray:[self.arraySelected firstObject][@"cities"]];
